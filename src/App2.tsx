		...
		Array.from(store.getState().nodeLookup.entries()).forEach(
			([nodeId, gnode]) => {
				...
				if (gnode.type === "group") {
					nodes.map((node) => {
						// 移動されたノードの位置を更新
						if (nodeId === node.id && gnode.position.x != node.position.x) {
							console.log(
								`違う:${node.id} gnode.position.x :${gnode.position.x}  node.position.x:${node.position.x}`,
							);
							node.position.x = gnode.position.x;
							node.position.y = gnode.position.y;
							return node;
						}
						console.log("同じ:");
						return node;
